...
from vnpy.extensions._minute_bar_internal import (
    minute_value,
...
from vnpy.bcolz_table.bar_reader import BarReader, NoDataForSid, NoDataOnDate
from common.utils.cli import maybe_show_progress
from common.utils.memoize import lazyval
from types import MappingProxyType as mappingproxy

def int_min(a, b): 
    return a if a <= b else b

def find_position_of_minute(market_opens, market_closes, minute_val, minutes_per_day, forward_fill):
        """
        ...
        return int((market_open_loc * minutes_per_day) + delta)
...
def minute_to_session_label(calendar, date, direction='next'):
    ...
    l = holidays.rollback(current - pd.Timedelta(days=1))
    r = holidays.rollforward(current + pd.Timedelta(days=1))
...
class BcolzMinuteBarMetadata(object):
    ...
    @classmethod
    def open(cls, meta_info):
        # raw_data = json.load(dumpstr)
        ...
        default_ohlc_ratio = meta_info.ohlc_ratio

        # if version >= 1:
        minutes_per_day = meta_info.minutes_per_day

        # if version >= 2:
        calendar = get_calendar(meta_info.calendar_name)
        start_session = pd.Timestamp(
            meta_info.start_session, tz='UTC')
        end_session = pd.Timestamp(meta_info.end_session, tz='UTC')
        # else:
        ...
        #     end_session = minute_to_session_label(calendar,
        #         pd.Timestamp(
...
class BcolzMinuteBarWriter(object):
    ...
    """
    COL_NAMES = ('open', 'open_int', 'high', 'low', 'close', 'volume')
    ...
    def __init__(self,
        ...
        self._schedule = calendar.schedule(
            self._start_session,
            self._end_session,
        )
    ...
    def pad(self, sid, date:pd.Timestamp):
        """
    ...
    def _write_cols(self, sid, dts, cols, invalid_data_behavior):
        ...
        tds = self._session_labels
        input_first_day = minute_to_session_label(
            self._calendar, 
        ...
        open_int_col = np.zeros(minutes_count, dtype=np.uint32)
...
class BcolzMinuteBarReader(MinuteBarReader):
    ...
    def __init__(self, rootdir, metadata:BcolzMinuteBarMetadata, sid_cache_sizes=_default_proxy):
    ...
    def last_available_dt(self):
        ...
        # nyse.schedule(start_date, start_date)
        schedule = self.calendar.schedule(self._end_session, self._end_session)
    ...
    def _minute_exclusion_tree(self):
        ...
        应从窗口中删除按每个位置范围的开始和结束键的间隔树。（这些是早期收盘价和分钟数之间的分钟数，
        如果没有提前关闭，则根据常规期间进行收盘价。
        每个节点的值与存储为元组的相同起始和结束位置。)

        数据以这种方式存储，以支持对问题的快速回答，给定的起始位置和结束位置是否与任何排除范围重叠？

        返回
        -------
        间隔树包含节点，这些节点表示由于提前关闭而要排除的分钟数。
        """
        itree = IntervalTree()
    ...
    def _exclusion_indices_for_range(self, start_idx, end_idx):
        ...
        （开始、停止）的元数列表，表示请求市场分钟窗口时应排除的分钟范围。
        """
    ...
    def get_value(self, sid, dt, field):
        ...
        try:
            ...
            value = 0
        if value == 0:
            if field == 'volume' or field == 'open_int':
                return 0
            else:
                return np.nan
    ...
    def _find_position_of_minute(self, minute_dt):
        ...
        内部方法，在第一个交易日开市以来，在每一个交易分钟的列表中返回给定分钟的位置。
        将非市场分钟数调整到最后一个收盘价。
        例如，如果 2002-01-02 是数据集的第一个交易日，则此方法将返回
        1: 2002-01-02 上午 9：32 东部。
        返回
        -------
        int：自市场在第一个交易日开盘以来的所有交易分钟列表中的给定分钟位置。
        """
    ...
    def load_raw_arrays(self, fields, start_dt, end_dt, sids):
        ...
        time_index = self.generate_time_range(start_idx, num_minutes)

        # time_index_ls = [self._pos_to_minute(i) for i in range(num_minutes)]
        for field in fields:
            if field != 'volume' or field != 'open_int':
            ...
            for i, sid in enumerate(sids):
                ...
                if field != 'volume' or field != 'open_int':
                    out[i, :len(where)][where] = (
                        values[where] * self._ohlc_ratio_inverse_for_sid(sid))
                else:
                    out[i, :len(where)][where] = values[where]

            fields_data.append(out)
        
        results = []
        for i, sid in enumerate(sids): 
            # df_data = dict(fields)
            # for j,field in enumerate(fields):
            #     df_data[field] = fields_data[i][field]
            df_data = dict(zip(fields, map(lambda fs:fs[i], fields_data)))
            df = pd.DataFrame( df_data, index=time_index, columns=fields)
            results.append(df)

        return results

    def generate_time_range(self, start_idx, num_minutes):
        
        if start_idx < 0:
            raise ValueError("start_dt必须在数据有效时间范围")
        count = num_minutes
        s_offset = start_idx
        time_index_ls = []
        for s in self._schedule.iterrows():
            st = s[1].market_open 
            et = s[1].market_close

            _minutes_per_day = int((et - st).seconds / 60)
            if s_offset != 0:
                if s_offset >= _minutes_per_day:
                    s_offset -= _minutes_per_day
                    continue
                else:
                    s_offset = 0
                    st += pd.Timedelta(minutes=s_offset)
            # 结尾处理
            if count < _minutes_per_day:
                et = st + pd.Timedelta(minutes=count)        
            time_index_ls.extend(pd.date_range(st, et - pd.Timedelta(minutes=1), freq='1min'))
            count -= _minutes_per_day
        
        return time_index_ls
